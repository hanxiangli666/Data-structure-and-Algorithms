...
template<typename V>
struct TrieNode {
    V val = NULL;                    // 该节点对应的值（为空表示不是完整的键）
    TrieNode<V>* children[256] = {};  // 256 个子节点指针，索引=字符 ASCII 码
};
```
...
public:
    ...
    vector<string> keysWithPrefix(string prefix); // 所有以 prefix 开头的键
    bool hasKeyWithPrefix(string prefix);        // 是否存在以 prefix 开头的键
...
struct TrieNode {
    TrieNode* children[26] = {};   // 只有 26 个字母
    bool isEnd = false;            // 是否是某个字符串的结尾
    int count = 0;                 // 经过这个节点的字符串数量（可选）
};
...
public:
    ...
    bool search(string word) {
        ...
        return node->isEnd;        // 到了末尾，但要检查是否是完整的键
    }
    ...
    bool startsWith(string prefix) {
        ...
        return true;               // 路径没断，说明有以 prefix 开头的键
    }
};
```
